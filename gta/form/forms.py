...
class RegisterForm(FlaskForm):
   ...
   # roles = [(r[0], r[1]) for r in resr]
    ...
    user_major = SelectField("Current Major", coerce=str, choices=majors, validators=[DataRequired()])
    user_degree = SelectField("Current Degree", coerce=str, choices=degrees, validators=[DataRequired()])
    user_gpa = DecimalField("GPA", places=2)
    user_hours = DecimalField("Hours Taken at UMKC", places=2)
    user_pass = PasswordField("Password",validators=[DataRequired()])
    user_confirm_pass = PasswordField("Confirm Password",validators=[DataRequired()])
